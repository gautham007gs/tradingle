...
function tradingle_enqueue_assets() {
	...
	wp_enqueue_style( 'tradingle-fonts', 'https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&family=Poppins:wght@400;500;600;700;800&family=Roboto:wght@400;500;700&display=swap', array(), null );
	...
	$critical_css = '.utility-bar{min-height:42px;background:#10263f}.site-header{position:sticky;top:0;z-index:1000;box-shadow:0 2px 8px rgba(0,0,0,.06)}.header-inner{min-height:84px;display:grid;grid-template-columns:minmax(200px,280px) minmax(0,1fr) auto}.hero-grid{display:grid;grid-template-columns:minmax(0,var(--content-width)) 1fr;gap:var(--layout-gap)}';
